...
void __fastcall TfrmBIorder::editamountKeyPress(TObject *Sender, wchar_t &Key)
{
	if (Key == VK_RETURN) {
		...
		if (aqorderdetail->FieldByName("needprocurenum")->AsInteger > 0) {
			logmessage = "修改订单" + aqupdatehead->FieldByName("OrderNtCode")->AsString + "书目《" + aqorderdetail->FieldByName("bookname")->AsString + "》的数量" + aqorderdetail->FieldByName("amount")->AsAnsiString + "为" + editamount->Text ;
			AddEvent(1,"一般订单窗口",UserID,StgID,logmessage,fcon);
		}
...
void __fastcall TfrmBIorder::editdetailbkKeyPress(TObject *Sender, wchar_t &Key)
{
	if (Key == VK_RETURN) {
		...
		logmessage = "修改订单" + aqupdatehead->FieldByName("OrderNtCode")->AsString + "书目《" + aqorderdetail->FieldByName("bookname")->AsString + "》的备注为" + editdetailbk->Text ;
		AddEvent(1,"一般订单窗口",UserID,StgID,logmessage,fcon);
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmBIorder::BtnDeleteClick(TObject *Sender)
{
	...
	if (aqorderdetail->FieldByName("ylocalnum")->AsInteger > 0 ) {
		MessageBoxA(0,"已确定锁定数量后不能删除，只能置为无效！","提示",MB_ICONWARNING);
		return;
	}
	if (MessageBoxA(0,"确认要删除吗？","提示",MB_ICONQUESTION|MB_OKCANCEL)==1)
	{
	//aqorderdetail->Edit();
...
void __fastcall TfrmBIorder::N6Click(TObject *Sender)
{
	...
	else
	{
		...
		if (newcatalogid > -1) {
        	...
        	sql = "update BS_OrderNote set state = 1,localnum = 0,bk = bk + '该书被更换' where id = " + aqorderdetail->FieldByName("id")->AsString ;
			TADOQuery *aq = new TADOQuery(this);
			aq->Connection = fcon;
			aq->Close();
			aq->SQL->Clear();
			...
			logmessage = "更换订单" + aqupdatehead->FieldByName("OrderNtCode")->AsString + "书目《" + ss + "》为《" + aqorderdetail->FieldByName("bookname")->AsString + "》";
			AddEvent(1,"一般订单窗口",UserID,StgID,logmessage,fcon);
		}
	}
}
...
void __fastcall TfrmBIorder::N7Click(TObject *Sender)
{
	...
	aqorderdetail->Locate("bkcatalogid",bkcatalogid,Opts);
	logmessage = "修改订单" + aqupdatehead->FieldByName("OrderNtCode")->AsString + "书目《" + aqorderdetail->FieldByName("bookname")->AsString + "》为有效";
	AddEvent(1,"一般订单窗口",UserID,StgID,logmessage,fcon);
...
void __fastcall TfrmBIorder::N8Click(TObject *Sender)
{
	...
	aqorderdetail->Locate("bkcatalogid",bkcatalogid,Opts);
	logmessage = "修改订单" + aqupdatehead->FieldByName("OrderNtCode")->AsString + "书目《" + aqorderdetail->FieldByName("bookname")->AsString + "》为无效";
	AddEvent(1,"一般订单窗口",UserID,StgID,logmessage,fcon);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmBIorder::dgdetailCellClick(TColumnEh *Column)
{
	...
	if (aqorderdetail->FieldByName("state1")->AsAnsiString == "有效" && labstate->Visible == false) {
		editamount->Enabled = true;
		editdiscount->Enabled = true;
	}
	else
...
void __fastcall TfrmBIorder::edclientbkKeyPress(TObject *Sender, wchar_t &Key)
{
	if (Key == VK_RETURN) {
		...
		logmessage = "修改订单" + aqupdatehead->FieldByName("OrderNtCode")->AsString + "书目《" + aqorderdetail->FieldByName("bookname")->AsString + "》的客户回告为" + edclientbk->Text ;
		AddEvent(1,"一般订单窗口",UserID,StgID,logmessage,fcon);
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmBIorder::dgdetailDblClick(TObject *Sender)
{
	...
	TfrmNewBookModify * frm = new TfrmNewBookModify(Application,fcon,aqorderdetail->FieldByName("bkcatalogid")->AsInteger,m_storageID,m_userID);
	if (frm->ShowModal() == mrOk ) {
		//logmessage = "修改订单" + aqupdatehead->FieldByName("OrderNtCode")->AsString + "书目《" + aqorderdetail->FieldByName("bookname")->AsString + "》的信息";
		//AddEvent(1,"一般订单窗口",UserID,StgID,logmessage,fcon);
	}
	delete frm;
	RefreshDetail();
