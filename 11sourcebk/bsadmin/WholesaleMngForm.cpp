...
void TfrmWholesaleMng::Init(LandInfo* li)
{
	...
	ChangeBsmode(BSNote);
	sphunhedabao->Connection = m_con; //混合打包
...
void __fastcall TfrmWholesaleMng::edtCatalogKeyPress(TObject *Sender, wchar_t &Key)
{
	...
	{
		if (BtnSave->Enabled) {
			MessageBox(0,"请先保存批销单！","一般发货" ,MB_ICONEXCLAMATION);
			return;
		}
		...
		else
		{
			...
			if (m_catalogSearchMode == 6) {
				try {
					...
					MessageBox(0,"请输入正确的定价！","一般发货" ,MB_ICONEXCLAMATION);
					return;
				}
			}
	...
	case OPRecCash:
		...
		if (dsetNtHeader->Active && dsetNtHeader->RecordCount >= 1)
		{
			...
			if (StrToDate(dsetNtHeader->FieldByName("HdTime")->AsAnsiString ) != Date()  ) {
				MessageBox(0,"现场收款只能收当日的！","一般发货" ,MB_OK);
				return;
			}
...
bool __fastcall TfrmWholesaleMng::DbgridToExcel(TRzDBGrid* dbg)
{
	...
	savedlg->FileName = StringReplace(edtclient->Text+editdanhao->Text ,"\\","-",TReplaceFlags()<<rfReplaceAll) + "发货单";
	if (savedlg->Execute())
	{
		String DBPath,Name;
	...
	*/
	...
	for(int q=0;q<dbg->FieldCount-1 ;++q)
	{
		if (dbg->Columns->Items[q]->Visible == true) {
			...
			/* (printmodle == 0 && (dbg->Columns ->Items [q]->Title ->Caption == "折扣" ||dbg->Columns ->Items [q]->Title ->Caption == "实洋")) {
	...
	for(int   i=2+n;i<=t2+1+n ;i++)
	{
		...
		for(int j=1;j<dbg->Columns ->Count  ;j++)
		{
			if (dbg->Columns->Items[j-1]->Visible == true) {
				if (dbg->Columns ->Items [j-1]->FieldName == "ISBN" || dbg->Columns ->Items [j-1]->FieldName == "内部识别码") {
					t1++;
	...
	if (dsetNtHeader->Active && dsetNtHeader->RecordCount >= 1)
	{
		...
		if (ret > 0)//插入成功
		{
			...
			if (dsetNtHeader->FieldByName("printcount")->AsInteger > 0 ) {
				if (ChangeDisplay) {
					logmessage = "添加一般发货单" + dsetNtHeader->FieldByName("WsaleNtCodeStr")->AsAnsiString ;
				}
				else
				{
					logmessage = "添加一般发货单" + dsetNtHeader->FieldByName("WsaleNtCode")->AsAnsiString ;
				}
...
void TfrmWholesaleMng::UpdateNote()
{
	if (dsetNote->Active && dsetNote->RecordCount >= 1)
	{
        ...
        if (dsetNtHeader->FieldByName("printcount")->AsInteger > 0 ) {
			...
			if (ss != "") {
            	if (ChangeDisplay) {
					logmessage = "修改一般发货单" + dsetNtHeader->FieldByName("WsaleNtCodeStr")->AsAnsiString ;
				}
				else
				{
					logmessage = "修改一般发货单" + dsetNtHeader->FieldByName("WsaleNtCode")->AsAnsiString ;
				}
				logmessage = logmessage + "的书目《" + dsetNote->FieldByName("CatalogName")->AsAnsiString + "》";
...
bool  TfrmWholesaleMng::discountcatch(int stkid)
{
	...
	if (query->FieldByName("value")->AsBoolean == false ) {
		...
		if (dbnbedtDiscount->Value < query->FieldByName("Discount")->AsFloat ) {
			MessageBox(0,"批销折扣低于成本折扣，不能发货！","提示",MB_ICONWARNING);
			return false;
		}
	}
	...
	if (query->FieldByName("value")->AsBoolean == true ) {
		...
		query->Open();
		if (dbnbedtDiscount->Value < query->FieldByName("Discount")->AsFloat ) {
			MessageBox(0,"批销折扣低于成本折扣！","提示",MB_ICONWARNING);
		}
	}
	...
	if (query->FieldByName("value")->AsBoolean == false ) {
		...
		if (dbnbedtDiscount->Value < query->FieldByName("wsalediscount")->AsFloat ) {
			MessageBox(0,"批销折扣低于正常折扣，不能发货！","提示",MB_ICONWARNING);
			return false;
		}
	}
	...
	if (query->FieldByName("value")->AsBoolean == true ) {
		...
		query->Open();
		if (dbnbedtDiscount->Value < query->FieldByName("wsalediscount")->AsFloat ) {
			MessageBox(0,"批销折扣低于正常折扣！","提示",MB_ICONWARNING);
		}
	}
...

{
	if (Key == '\r') {
		if (edtclient->Text =="") {
			MessageBoxA(0,"请选择单位！","提示",MB_ICONINFORMATION);
			return;
		}
				  ...
				  if (frm->printnow) {
								  try
								   ...
								   }catch(...)
								   {
									  ...
									  MessageBox(Handle ,"打印失败，请重试！","提示",MB_ICONEXCLAMATION);
									   delete frm;
									   return;
								   }
...
void TfrmWholesaleMng::muilprint(AnsiString HeaderID,int type,bool xiaopiao)
{
   	...
   	Tfrmwsaleprinta* frm = new Tfrmwsaleprinta(this,m_con);
		...
		frm->address = address;
				...
				if ((type==103) ||(type==104)) { //发货检货联打
				 frm->jianhuodanprint(HeaderID,xiaopiao);
				}else if ((type==203)||(type==204)) {//面单打印
				   frm->miandanprint(HeaderID,xiaopiao);
				}else if ((type==205)||(type==206)) {  //晓票打印或预览
				 frm->xiaopiaoliandaprint(HeaderID,xiaopiao);
				}else if ((type==207)||(type==208)) {  //A5打印或预览
...
{
  if (cborder->Text!="") {
	AnsiString sql="";  ///检查订单中的书目ID是否与发货单相一致
	AnsiString st;
	...
	st = sp->Parameters->ParamByName("@returnvalue")->Value;
	   ...
	   MessageBox(Handle,st.c_str(),"提示",MB_ICONWARNING);
