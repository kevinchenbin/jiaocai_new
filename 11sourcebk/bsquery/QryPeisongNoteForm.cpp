...
bool __fastcall TfrmQryPeisongNote::DbgridToExcel(TDBGridEh* dbg)
{
  ...
  //	savedlg->FileName = "配送查询";

		 savedlg->FileName = "配送查询"+FormatDateTime("yyyy-mm-ddHHMM",Now())+".csv";
...
void __fastcall TfrmQryPeisongNote::dblkpcbbBookTypeClick(TObject *Sender)
{
...
void __fastcall TfrmQryPeisongNote::dbgrdByNtHeaderDblClick(TObject *Sender)
{
	...
	if (dsetByNtHeader->RecordCount > 0) {
		TfrmNotedetail *frm = new TfrmNotedetail(Application,dsetByNtHeader->FieldByName("Code")->AsString,6,fcon,0,m_userID);
		frm->ShowModal();
		delete frm;
	}
}
