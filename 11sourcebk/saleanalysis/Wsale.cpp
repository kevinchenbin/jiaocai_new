...
TfrmWsale *frmWsale;
//---------------------------------------------------------------------------
__fastcall TfrmWsale::TfrmWsale(TComponent* Owner,TADOConnection *cn,int stgid)
	: TForm(Owner)
{
   aqday->Connection = cn;
...
//---------------------------------------------------------------------------
void __fastcall TfrmWsale::BtnViewClick(TObject *Sender)
{
  String  wherestr;
  wherestr = " where 1 = 1";
   ...
   if (chstorage->Checked ) {
		...
		wherestr = wherestr + " and BS_WsaleNoteHeader.StgID = " + IntToStr(fstgid);
   }

   if (rbback->Checked) {
	 wherestr = wherestr + " and A.amount < 0 ";

   }else if (rbwsale->Checked ) {
...
void TfrmWsale::Day(AnsiString wherestr)
{
    	String sqlstr;
	float Tfix,Tdis,Tamount,TCbprice,Tprofite;

	sqlstr = "select cast(year(hdtime) as varchar(4))+'-'+ cast(month(hdtime) as varchar(2))+'-'+ cast(day(hdtime) as varchar(2)) as date ,sum(A.FixedPrice) fix,sum(A.DiscountedPrice) dis,sum(A.Amount) amount,sum(A.Amount*B.Cbprice) as Cbprice,sum(A.DiscountedPrice-A.Amount*B.Cbprice) as profite "
			 " from BS_WsaleNote A left join BS_WsaleNoteHeader on  A.WsaleNtHeaderID = BS_WsaleNoteHeader.id left join STK_BookInfo B on A.BkInfoID = B.ID " ;
	sqlstr = sqlstr + wherestr + " group by year(hdtime),month(hdtime), day(hdtime)";
	sqlstr = sqlstr + " UNION select '合计',sum(A.FixedPrice) fix,sum(A.DiscountedPrice) dis,sum(A.Amount) amount,sum(A.Amount*B.Cbprice) as Cbprice,sum(A.DiscountedPrice-A.Amount*B.Cbprice) as profite "
			 " from BS_WsaleNote A left join BS_WsaleNoteHeader on  A.WsaleNtHeaderID = BS_WsaleNoteHeader.id left join STK_BookInfo B on A.BkInfoID = B.ID " ;
...
void TfrmWsale::Month(AnsiString wherestr)
{
    String sqlstr;
	float Tfix,Tdis,Tamount,TCbprice,Tprofite;

	sqlstr = "select cast(year(hdtime) as varchar(4))+'-'+ cast(month(hdtime) as varchar(2)) as date,sum(A.FixedPrice) fix,sum(A.DiscountedPrice) dis,sum(A.Amount) amount,sum(A.Amount*B.Cbprice) as Cbprice,sum(A.DiscountedPrice-A.Amount*B.Cbprice) as profite "
			 " from BS_WsaleNote A left join BS_WsaleNoteHeader on  A.WsaleNtHeaderID = BS_WsaleNoteHeader.id left join STK_BookInfo B on A.BkInfoID = B.ID " ;
...
//---------------------------------------------------------------------------
void __fastcall TfrmWsale::pgChange(TObject *Sender)
{
   if (query ) {
       BtnView->Click();
   }
...
void __fastcall TfrmWsale::BtnExitClick(TObject *Sender)
{
  Close();
}
//---------------------------------------------------------------------------

void __fastcall TfrmWsale::FormClose(TObject *Sender, TCloseAction &Action)
{
  Action = caFree ;
}
//---------------------------------------------------------------------------
void __fastcall TfrmWsale::BtnPrintClick(TObject *Sender)
{
 /*
   if (pg->ActivePage == tsday) {
   ...
   if (rbwsale->Checked ) {
		...
		if (pg->ActivePage == tsday) {
			...
			frm->aqtitle->Caption = "批销按日统计报表";
			RPGrid(dgday,frm->qrdg);
		}else
		{
			frm->aqtitle->Caption = "批销按月统计报表";
			RPGrid(dgmonth ,frm->qrdg);
		}
   }else
   {
		if (pg->ActivePage == tsday) {
			frm->aqtitle->Caption = "批销退货按日统计报表";
				RPGrid(dgday,frm->qrdg);
		}else
		{
			frm->aqtitle->Caption = "批销退货按月统计报表";
			RPGrid(dgmonth ,frm->qrdg);
		}
   }
...
/*void TfrmWsale::RPGrid(TRzDBGrid *dg,TQRPGrid *PRdg )
{
   int rows,cols;
...
void __fastcall TfrmWsale::BtnExportClick(TObject *Sender)
{
	if (pg->ActivePage == tsday) {
...
//Excel导出函数
bool __fastcall TfrmWsale::DbgridToExcel(TRzDBGrid* dbg)
{
	AnsiString temptext ;
...
//  在Excel中成字符串显示
	if (rbwsale->Checked) {
		v.OlePropertyGet("Cells",1,3).OlePropertySet("Value","批销" );
	}
	if (rbback->Checked) {
		v.OlePropertyGet("Cells",1,3).OlePropertySet("Value","批销退货" );
	}
...
void __fastcall TfrmWsale::dgmonthTitleClick(TColumn *Column)
{
	if (aqmonth->IsEmpty() ) {
		return;
	}
...
void __fastcall TfrmWsale::dgdayTitleClick(TColumn *Column)
{
	if (aqday->IsEmpty() ) {
        return;
	}
...
void __fastcall TfrmWsale::BtnAlignBottomClick(TObject *Sender)
{
WindowState = wsMinimized   ;
}
//---------------------------------------------------------------------------

void __fastcall TfrmWsale::FormKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
...
void __fastcall TfrmWsale::dtpstartChange(TObject *Sender)
{
	cbstart->Checked = true;
}
//---------------------------------------------------------------------------

void __fastcall TfrmWsale::dtpendChange(TObject *Sender)
{
	cbend->Checked = true;
}
//---------------------------------------------------------------------------
