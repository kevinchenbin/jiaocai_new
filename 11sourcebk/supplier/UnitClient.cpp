...
//---------------------------------------------------------------------------
__fastcall Tfrmclient::Tfrmclient(TComponent* Owner,TADOConnection *cn,int astgid)
	: TForm(Owner)
{
   fcon = cn;
...
void Tfrmclient::ReadyData()
{
	AnsiString sql;
	if (querywhere) {
		sql = " exec USP_BS_Client_View 1,0,'"+IntToStr(fstgid)+"','" + sqlwhere + "'";
	}
	else
	{
