...
var
  ...
  begin
    ...
    DrawFlag := AlignFlags[Alignment];
    if DrawColumn.WordWrap and DrawColumn.CurLineWordWrap(RowHeights[ARow]) then
      DrawFlag := DrawFlag or DT_WORDBREAK or RTL[UseRightToLeftReading];
  ...
  begin
    ...
    if CheckPrevRow then
    begin
      if ViewScroll and (AreaRow > TopRow - TopDataOffset) then
      begin
    ...
    begin
      if ViewScroll and (AreaRow < FIntMemTable.InstantReadRowCount-1) then
      begin
        InstantReadRecordEnter(AreaRow+1);
        try
          Result := CheckColumnValue;
...
begin
  ...
  if (DataLink = nil) or not DataLink.Active or
   (AreaRow < 0) or (AreaRow >= InstantReadRecordCount) then
  begin
    Canvas.FillRect(ARect);
    Exit;
  end;
  ...
  if (FIntMemTable <> nil) and (AreaCol = 0) and FIntMemTable.MemTableIsTreeList then
  begin
    DrawCellTreeArea(AreaCol, AreaRow, AState, ARect, The3DRect);
    ARect.Left := ARect.Left + GetCellTreeElmentsAreaWidth;
//    if The3DRect then
//      ARect.Left := ARect.Left - 1;
  end;
...
//      InflateRect(ATextRect, -1, -1);
  ...
  Highlight := HighlightDataCellColor(AreaCol, AreaRow,
    Value, AState, FColCellParamsEh.FBackground, Canvas.Font);
  if Highlight then
    AState := AState + [gdSelected];
  Canvas.Brush.Color := FColCellParamsEh.FBackground;
  ...
  if DrawColumn.HideDuplicates then
  begin
    if (AreaRow < InstantReadRecordCount) and
       (DrawColumn.FCheckedDuplicates[AreaRow-TopRow+TopDataOffset+1] = False)
    then
      if CheckColumnDuplicateValues(DrawColumn, FColCellParamsEh.FText, False)
      then
        ClearSubLine := True;
    ...
    begin
      FColCellParamsEh.FText := '';
      FColCellParamsEh.FImageIndex := -1;
      if DrawColumn.FCheckedDuplicates[AreaRow-TopRow+TopDataOffset] = False then
      begin
        ClearOnLine := True;
        DrawColumn.FCheckedDuplicates[AreaRow-TopRow+TopDataOffset] := True;
      end;
    end;
  end;
  ...
  if DefaultDrawing and (ARect.Right > ARect.Left) then
  begin
    IsFillTextCell := The3DRect and not (gdSelected in AState) and
      (ADrawByThemes or (Flat and ThemesEnabled));
    if IsFillTextCell then
      FillCellRect(ADrawByThemes, ARect, False, Highlight, EmptyRect, The3DRect);
    if (DrawColumn.ImageList <> nil) and DrawColumn.ShowImageAndText then
    begin
      ARect1 := ARect;
      ARect1.Right := ARect1.Left + DrawColumn.ImageList.Width + 4;
    ...
    if DrawColumn.GetColumnType in [ctCommon..ctKeyPickList] then
      with FColCellParamsEh do
        WriteCellText {WriteTextEh}(DrawColumn, Canvas, ARect, not IsFillTextCell,
          XFrameOffs, YFrameOffs, FText,
          FAlignment, DrawColumn.Layout, DrawColumn.WordWrap and DrawColumn.CurLineWordWrap(RowHeights[ARow]),
          DrawColumn.EndEllipsis, 0, 0)
    else if DrawColumn.GetColumnType = ctKeyImageList then
    begin
      if not IsFillTextCell then
    ...
    begin
      ...
      PaintButtonControl {Eh}(Canvas.Handle, ARect1, Canvas.Brush.Color, bcsCheckboxEh,
        0, Flat, False, True, FColCellParamsEh.FCheckboxState);
    end else if DrawColumn.GetColumnType = ctGraphicData then
      DrawGraphicCell(DrawColumn, ARect, Canvas.Brush.Color);
  end;
  if DrawColumn.AlwaysShowEditButton then // Draw edit button
  begin
    SetRect(ARect1, ARect.Right, ARect.Top,
    ...
    {if AEditStyle = esUpDown then
      ...
      PaintInplaceButton(DrawColumn, Canvas, EditToButtonStyle[AEditStyle], ARect1, ACellRect,
        0, False, Flat, DataLink.Active, Canvas.Brush.Color, nil);
      if Flat and The3DRect then
        Canvas.FillRect(Rect(ARect1.Left - 1, ARect1.Top, ARect1.Left, ARect.Bottom));
...
//    if FInplaceEditorButtonHeight < FStdDefaultRowHeight then
    if FInplaceEditorButtonHeight < (ARect.Bottom - ARect.Top) then
      Canvas.FillRect(Rect(ARect.Right, ARect1.Bottom, ARect.Right + AButtonsWidth, ARect.Bottom));
  end;
  PaintRequiredState(DrawColumn, FColCellParamsEh.Text, ARect);
  if Columns.State = csDefault then
    OldDrawDataCell(ARect, DrawColumn.Field, AState);
  DrawColumnCell(ARect, AreaCol, DrawColumn, AState);
  if FInplaceSearching
//    and (gdSelected in AState) KMV #12
    and (gdFocused in AState)
    and IsCurrentRow(FOldActiveRecord)
    and (ACol = Col)
    and ((dgAlwaysShowSelection in Options) or Focused)
    and not (csDesigning in ComponentState)
    ...
    begin
      ...
      if FInplaceEditorButtonHeight < FStdDefaultRowHeight then
        Canvas.FillRect(Rect(FilterRect.Left, FilterRect.Bottom, FilterRect.Right,
           FilterRect.Bottom + (FStdDefaultRowHeight - FInplaceEditorButtonHeight) ));
